...
module.exports = (Model, schema = {}) => {
        ...
        .then(validatedData => {
          ...
          switch (attachTo) {
            case 'data':
              ...
              break
            case 'instance':
              const schema = Model.getAdvancedValidationSchema()
              if (!performPartialValidation && schema) {
                const schemaKeys = Object.keys(schema.fields)
                const instanceKeys = Object.keys(ctx.instance.toObject())
                const idName = Model.getIdName()
                instanceKeys.forEach(key => {
                  if (!schemaKeys.includes(key) && key !== idName) {
                    delete ctx.instance[key]
  ...
  Model.getAdvancedValidationSchema = function() {
    return _.get(this, 'definition.settings.validation.schema')
  }
}
